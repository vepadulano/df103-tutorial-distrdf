...
##
# \date August 2022
# \authors Vincenzo Eduardo Padulano (UPV, CERN)
import argparse
import os
import shlex
import subprocess
...
parser.add_argument("dataset", help="Type of dataset to use.", choices=["full", "skimmed"])
...
def create_connection() -> Client:
    cluster = LocalCluster(n_workers=os.cpu_count()//2, threads_per_worker=1, processes=True)
...
def df103_NanoAODHiggsAnalysis(connection: Client, path: str):
    # Create dataframes for signal, background and data samples

    # Signal: Higgs -> 4 leptons
    df_sig_4l = RDataFrame("Events", os.path.join(path, "SMHiggsToZZTo4L.root"), daskclient=connection)

    # VERY IMPORTANT
    # Distribute header with C++ to the backend
    # Can be done only once, on the first RDF defined
    df_sig_4l._headnode.backend.distribute_headers("header.hxx")
    # VERY IMPORTANT
    ...
    # tutorial.
    df_bkg_4mu = RDataFrame("Events", os.path.join(path, "ZZTo4mu.root"), daskclient=connection)
    df_bkg_4el = RDataFrame("Events", os.path.join(path, "ZZTo4e.root"), daskclient=connection)
    df_bkg_2el2mu = RDataFrame("Events", os.path.join(path, "ZZTo2e2mu.root"), daskclient=connection)

    # CMS data taken in 2012 (11.6 fb^-1 integrated luminosity)
    df_data_doublemu = RDataFrame("Events", [os.path.join(path, f)
                                             for f in ["Run2012B_DoubleMuParked.root", "Run2012C_DoubleMuParked.root"]], daskclient=connection)
    df_data_doubleel = RDataFrame("Events", [os.path.join(path, f)
                                             for f in ["Run2012B_DoubleElectron.root", "Run2012C_DoubleElectron.root"]], daskclient=connection)
    ...
    # RunGraphs([df_h_sig_4mu, df_h_bkg_4mu, df_h_data_4mu,
    #            df_h_sig_4el, df_h_bkg_4el, df_h_data_4el,
    #            df_h_sig_2el2mu, df_h_bkg_2el2mu, df_h_data_2el2mu])

    # Get histograms (does not rerun the event loop if RunGraphs was called)
    ...
    # Make plots
    plot(signal_4mu, background_4mu, data_4mu, "m_{4#mu} (GeV)", "higgs_4mu.png")
    plot(signal_4el, background_4el, data_4el, "m_{4e} (GeV)", "higgs_4el.png")
    plot(signal_2el2mu, background_2el2mu, data_2el2mu, "m_{2e2#mu} (GeV)", "higgs_2el2mu.png")
    ...
    plot(h_sig_4l, h_bkg_4l, h_data_4l, "m_{4l} (GeV)", "higgs_4l.png")


def download_data(dir: str):
    cmd = "bash download_data.sh"
    filenames = ["SMHiggsToZZTo4L.root", "ZZTo4mu.root", "ZZTo4e.root", "ZZTo2e2mu.root", "Run2012B_DoubleMuParked.root",
                 "Run2012C_DoubleMuParked.root", "Run2012B_DoubleElectron.root", "Run2012C_DoubleElectron.root"]
    if not all(os.path.exists(os.path.join(dir, filename)) for filename in filenames):
        print(f"Downloading data to {dir}")
        subprocess.run(shlex.split(cmd), cwd=dir, check=True)
    else:
        print(f"Data already present at destination {dir}")


if __name__ == "__main__":
    args = parser.parse_args()
    path = os.path.join(os.path.dirname(os.getcwd()), f"data-{args.dataset}")
    # Download data first
    download_data(path)
    # Run the analysis
    print("Starting the analysis")
    with Client(LocalCluster(n_workers=os.cpu_count()//2, threads_per_worker=1, processes=True)) as connection:
        df103_NanoAODHiggsAnalysis(connection, path)
